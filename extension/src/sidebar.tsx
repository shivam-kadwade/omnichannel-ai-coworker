...
function CoworkerSidebar() {
  ...
  useEffect(() => { browser.storage.local.get("sidebarTheme").then(({ sidebarTheme }) => { if (sidebarTheme === "light" || sidebarTheme === "dark") setTheme(sidebarTheme); }); }, []);
  const toggleTheme = () => setTheme(current => { const next = current === "light" ? "dark" : "light"; browser.storage.local.set({ sidebarTheme: next }); return next; });
